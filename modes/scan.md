...
### Level 1 - Direct Playwright (MAIN)
...
- It sees the page in real time (not Google-cached results).
- It works with SPAs (Ashby, Lever, Workday).
- It catches new offers instantly.
- It does not depend on Google's indexing.

**Every company MUST have a `careers_url` in portals.yml.** If it does not, find it once, save it, and reuse it in future scans.

### Level 2 - Greenhouse API (COMPLEMENTARY)

For companies on Greenhouse, the JSON API (`boards-api.greenhouse.io/v1/boards/{slug}/jobs`) returns clean structured data. Use it to complement Level 1: faster than Playwright but Greenhouse-only.

### Level 3 - WebSearch queries (WIDE DISCOVERY)

The `search_queries` with `site:` filters cover portals broadly (all Ashby, all Greenhouse, etc.). Useful to discover NEW companies not yet in `tracked_companies`, but results can be stale.
...
The levels are additive: all run, results merge and deduplicate.
...
5. **Level 2 - Greenhouse APIs** (parallel):
   ...
   a. WebFetch the API URL, get JSON with the job list.
   b. For each job extract: `{title, url, company}`.
   c. Accumulate into the candidate list (dedup vs Level 1).
...
6. **Level 3 - WebSearch queries** (parallel when possible):
   ...
   c. Accumulate into the candidate list (dedup vs Level 1+2).

7. **Title filter** via `title_filter` in `portals.yml`:
   - At least 1 keyword from `positive` must appear in the title (case-insensitive).
   - 0 keywords from `negative` may appear.
   - `seniority_boost` keywords boost priority but are not required.
...
8.5. **Verify liveness of Level-3 WebSearch results** BEFORE adding to pipeline:
   ...
   WebSearch results can be stale (Google caches for weeks or months). To avoid evaluating expired offers, verify each new Level-3 URL with Playwright. Levels 1 and 2 are inherently real-time and do not need this check.
   ...
   **Do not stop the whole scan if one URL fails.** If `browser_navigate` errors (timeout, 403, etc.), mark it as `skipped_expired` and continue.

9. **For each verified new offer that passes the filters**:
...
- **Ashby**: `"Senior AI PM (Remote) @ EverAI"` gives title: `Senior AI PM`, company: `EverAI`.
- **Greenhouse**: `"AI Engineer at Anthropic"` gives title: `AI Engineer`, company: `Anthropic`.
- **Lever**: `"Product Manager - AI @ Temporal"` gives title: `Product Manager - AI`, company: `Temporal`.

Generic regex: `(.+?)(?:\s*[@|-]\s*|\s+at\s+)(.+?)$`.

## Private URLs

If a URL is not publicly reachable:
...
Every company in `tracked_companies` must have a `careers_url`: the direct link to its jobs page. This avoids searching for it every time.
