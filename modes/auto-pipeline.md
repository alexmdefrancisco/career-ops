...
3. **WebSearch (last resort):** look up role title + company on secondary portals that index the JD as static HTML.
...
1. **Extract form questions**: use Playwright to navigate the form and snapshot it. If extraction fails, use the generic questions.
...
Register in `data/applications.md` with all columns including Report and PDF set to the green check.
