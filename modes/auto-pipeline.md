...
## Step 3 - Generate PDF and Cover Letter (ALWAYS)
...
- If `"latex"`, run the full pipeline in `modes/latex.md`
- Otherwise (default), run the full pipeline in `modes/pdf.md`
...
1. **Extract form questions**: use Playwright to navigate to the form and snapshot. If they cannot be extracted, use the generic questions.
...
Register in `data/applications.md` with all columns including Report and PDF set to ✅.
