...
Maps non-canonical statuses to their canonical equivalents and strips markdown bold and dates from the status column. Spanish aliases like `Enviada` become `Applied`, `CERRADA` becomes `Discarded`, etc. `DUPLICADO` info is moved to the notes column.
