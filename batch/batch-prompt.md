...
**RULE: For article metrics, article-digest.md prevails over cv.md.** cv.md may carry older numbers — that is fine.
...
**Cross-cutting advantage**: frame the profile as **"Technical builder"** who adapts the framing to the role:
...
Make "builder" a professional signal, not "hobby maker". The framing changes, the truth stays the same.

#### Block A - Role Summary

MANDATORY Section A table. The dashboard parser reads these rows by exact label; if a row is missing, the report card on the dashboard is blank.
...
- `| Seniority | … |`
- `| Remote | … |` - **required**
- `| Comp | … |` - **required** (estimated band or `n/d` if not applicable)
- `| Team size | … |` (optional)
- `| TL;DR | … |` - **required**

**Report header (MANDATORY, all fields):**
...
`TL;DR` is repeated in the header, not just inside the table. The dashboard and `verify-pipeline.mjs` fail if any listed field is missing.

#### Block B — CV Match

Read `cv.md`. Build a table mapping each JD requirement to exact CV lines or i18n.ts keys.
...
2. **"Sell senior without lying" plan**: specific lines, concrete wins, founder as an advantage
3. **"If they downlevel me" plan**: accept if comp is fair, 6-month review, clear criteria

#### Block D — Comp and Demand

Use WebSearch for current salaries (Glassdoor, Levels.fyi, Blind), company comp reputation, demand trend. Table with data and cited sources. If there is no data, say so.
...
| 6 | score | X.XX/5 | `4.55/5` | Or `N/A` if not scoreable |
| 7 | pdf | emoji | `✅` or `❌` | If a PDF was generated |
...
7. When writing English text (PDF summaries, bullets, STAR stories), use native tech English: short sentences, action verbs, no unnecessary passive voice, no "in order to" or "utilized"
